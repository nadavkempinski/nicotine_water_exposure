...
data_detects = data_clean |> # detections only
  filter(sample_concentration_normalized > 0, keep !="remove") |> # remove all 0 dots so we can just see the detected results
  mutate(sample_conc_log = log10(sample_concentration_normalized))

data_zeros = data_clean |> filter(keep !="remove") # includes non-detects
```
