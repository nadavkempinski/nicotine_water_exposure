...
ca_hydro_regions <- st_read("C:/Users/nadav/OneDrive/Desktop/Research/R/data/i03_Hydrologic_Regions/i03_Hydrologic_Regions.shp") |> st_transform(crs = ca_state_shp_crs)
...
# Additional map components to consider
...
# ca_gw_basins = st_read("C:/Users/nadav/Downloads/i08_B118_CA_GroundwaterBasins") |> st_transform(4326)
...
ca_base_map = ggplot() +
  ...
  geom_sf(data = ca_hydro_regions, color = "gray60", fill = NA, size = 0.2) +
...
ca_samples_map <- ca_base_map +
  ...
  scale_fill_gradientn(
    ...
    # ADDED: Move labels to left, put title on top, and set to 2nd position
  ...
  scale_shape_manual(
    ...
    # ADDED: Force analyte to be the 1st position
  ...
  theme(
    ...
    legend.position = c(0.7, 0.7),  # tweaked slightly lower to fit both stacked
    legend.box = "vertical",          # ADDED: Force legends to stack top-to-bottom
